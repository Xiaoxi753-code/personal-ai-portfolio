...
function ProjectCard({ index, title, description, tags }: ProjectCardProps) {
  return (
    <article className="group rounded-[1.75rem] border border-slate-200/80 bg-white p-6 shadow-[0_16px_45px_rgba(76,97,130,0.08)] transition duration-300 hover:-translate-y-2 hover:border-sky-200 hover:shadow-[0_24px_60px_rgba(56,142,220,0.16)] sm:p-8">
      ...
      <h3 className="mt-8 text-2xl font-semibold tracking-tight text-slate-800">{title}</h3>
    ...
    <main className="relative z-10 min-h-screen text-slate-800 selection:bg-sky-200 selection:text-sky-950">
      ...
      <section id="projects" className="bg-white">
        <div className="mx-auto max-w-6xl px-5 py-20 sm:px-8 sm:py-28 lg:px-10">
          <div className="max-w-2xl"><p className="font-mono text-sm tracking-[0.2em] text-sky-500 uppercase">Experience & Projects</p><h2 className="mt-4 text-3xl font-semibold tracking-tight text-slate-800 sm:text-5xl">经历与项目</h2><p className="mt-5 text-base leading-7 text-slate-500 sm:text-lg">这里将展示我在学习和实践中完成的代表作品。当前内容为结构占位，后续会替换成真实项目。</p></div>
          <div className="mt-12 grid gap-5 md:grid-cols-2">
            <ProjectCard index="01" title="个人知识管理实验室" description="一个用于整理学习笔记、项目经验与灵感的数字空间，探索如何让知识真正可查找、可连接、可复用。" tags={["Next.js", "Knowledge", "Design"]} />
            <ProjectCard index="02" title="智能生活助手" description="一个围绕日常计划与信息整理展开的 AI 助手概念项目，尝试把复杂任务变成清晰、友好的交互体验。" tags={["AI", "Product", "Prototype"]} />
          </div>
        </div>
      </section>

      <footer id="contact" className="bg-white">
        <div className="mx-auto flex max-w-6xl flex-col gap-8 border-t border-slate-100 px-5 py-16 sm:px-8 md:flex-row md:items-end md:justify-between lg:px-10"><div><p className="text-sm font-medium text-sky-500">保持联系</p><h2 className="mt-3 text-3xl font-semibold tracking-tight text-slate-800">一起创造点有意思的东西。</h2></div><div className="flex flex-wrap gap-3"><a href="https://github.com/your-username" target="_blank" rel="noreferrer" className="rounded-full border border-slate-200 px-5 py-2.5 text-sm text-slate-600 no-underline transition-colors duration-200 hover:border-sky-300 hover:text-sky-600">GitHub ↗</a><a href="mailto:hello@example.com" className="rounded-full border border-slate-200 px-5 py-2.5 text-sm text-slate-600 no-underline transition-colors duration-200 hover:border-violet-300 hover:text-violet-600">Email ↗</a></div></div>
